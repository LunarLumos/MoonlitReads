...
\begin{abstract}
Accurate identification of neuropeptides is important for understanding peptide-mediated signaling and for supporting peptide-oriented therapeutic discovery. Neuropeptides partic- ipate in a broad spectrum of biological activities, including neurotransmission, endocrine regulation, metabolism, stress response, and intercellular communication. Nevertheless, experimental identification of neuropeptides is labor-intensive, time-consuming, and not al- ways practical for large-scale screening. Therefore, an effective computational framework for sequence-based neuropeptide prediction is highly desirable. In this study, we propose a transformer-based multi-view prediction framework that integrates ProtBERT, ProtT5, and ESM-2 representations with hybrid machine learning and blend learning. First, three pre- trained protein language models are used to extract complementary contextual and semantic information from peptide sequences. Second, the resulting feature spaces are concatenated into a unified 3328-dimensional representation and standardized for downstream classifica- tion. Lastly, the four predictive models (SVM, XGBoost, LightGBM, and AttLSTM) were generated as independent outputs from training and testing of the models, and were subsequently combined using a leakage-free probability-based blending method. Results from these experiments highlighted that the performance of the aggregate transformer representation (i.e., after probability-based aggregation) consistently enhanced performance compared to using an individual representation for each model. Among the single-model classifiers, LightGBM produced the best empirical results when considered as a stand-alone classifier; whereas, the final multi-model blend produced the highest accuracy (0.9051) and F1-score (0.9052), as well as the highest MCC (0.8101), and the highest AUC (0.9592) for the independent test dataset. These findings suggest that using multi-view transformer representations along with aggregation by way of probability blending is an effective and reproducible method for neuropeptide prediction.
\end{abstract}
...
\begin{figure*}[!t]
    ...
    \caption{Systematic workflow of the proposed framework.}
    \label{fig:1}
\end{figure*}

\section{Methodology}
\subsection{Dataset construction}
The experiments are conducted on a balanced benchmark dataset consisting of training and independent test sets. The total sample size of 3,880 sequences has 1,940 samples from both the positive and negative groups in the training split while, in the independent testing split, there are 990 samples with 495 samples for each of the positive and negative groups. The evaluation of sensitivity (true positive rate) and specificity (true negative rate) is therefore fair due to the equal number of samples in each test; this reduces the risk of threshold-dependent reporting that is biased.
...
To capture complementary sequence information, three pretrained protein language model embeddings are used as separate feature views. Each model generates a numerical representation of the same peptide samples, thereby enabling both single-view evaluation and multi-view fusion. The final fused representation is obtained through horizontal concatenation of ProtT5, ProtBERT, and ESM-2 descriptors, resulting in a 3328-dimensional feature vector per sample.\textbf{ The feature extraction and fusion process should be illustrated in Fig. 2.}

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth]{fig2_embedding_fusion.png}
    \caption{Systematic workflow of the proposed framework.}
    \label{fig:1}
\end{figure*}
...
ProtBERT is used as the first sequence representation method. It provides contextual embeddings derived from transformer-based pretraining on large protein databases [8,10]. In the overall workflow, ProtBERT contributes a 1024-dimensional feature block to both the single-view experiments and the final fused representation. Feature extraction is performed by forwarding tokenized sequences through \texttt{Rostlab/prot\_bert} and selecting the final hidden-state vector of the CLS token. In this study, ProtBERT serves as a baseline semantic representation for evaluating single-view predictive performance.
...
To determine model performance, accuracy, sensitivity, specificity, precision, F1-score, MCC, and AUC are used. Accuracy measures the total number of correct classifications. Sensitivity indicates how well the model identifies all positive samples and specificity indicates how well the model identifies all negative samples. Precision measures how reliable the positive class is, while F1-score combines both precision and recall to measure how reliable the positive class is. MCC provides a balanced measure of correlation between the predicted labels and actual labels in the presence of class imbalance. AUC represents class-independent discrimination ability of the predictive model. Using these seven metrics allows for a thorough evaluation of predictive performance.

In the evaluation workflow, all metrics are computed from the confusion matrix generated on the independent test split. Probabilistic outputs are obtained from posterior class probabilities for SVM, XGBoost, and LightGBM, and from softmax-normalized outputs for AttLSTM. This unified metric computation allows direct comparison between classical learners, deep learning, and ensemble fusion models. \textbf{A ROC-based visual summary of the single-view models should be presented in Fig. 4.}
...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth]{fig4_single_view_auc_heatmap.png}
    \caption{Systematic workflow of the proposed framework.}
    \label{fig:1}
\end{figure*}
...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth]{fig6_best_blend_confusion_matrix.png}
    \caption{Systematic workflow of the proposed framework.}
    \label{fig:1}
\end{figure*}
...
The AttLSTM model used a customized neural architecture rather than relying on default PyTorch Model Builder configurations. Specifically, the network consists of a two-layer bidirectional LSTM with \texttt{hidden\_dim=256}, \texttt{dropout=0.3}, and \texttt{batch\_first=True}, followed by a learned attention layer. This is followed by a fully connected layer of size 512 to 256 with ReLU activation and \texttt{dropout=0.3}, and finally an output layer of size 256 to 2. Training was performed using \texttt{CrossEntropyLoss} and the \texttt{AdamW} optimizer with learning rate \texttt{0.001}, \texttt{batch\_size=64}, and \texttt{epochs=50}. The DataLoader was constructed with deterministic shuffling using a seeded random generator, and the global random seed was fixed at \texttt{42} for both NumPy and PyTorch.
